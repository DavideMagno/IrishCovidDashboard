...
# MAP ---------------------------------------------------------------------
    ...
    output$total.ratio <- renderText({
        Data$covid.ireland %>% 
            dplyr::select(Date, Ratio_Total) %>% 
            CalculateMA(input$date)
    })
    
    output$icu.ratio <- renderText({
        Data$covid.ireland %>% 
            dplyr::select(Date, `Ratio_In ICU`) %>% 
            CalculateMA(input$date)
    })
    
    output$dead.ratio <- renderText({
        Data$covid.ireland %>% 
            dplyr::select(Date, Ratio_Dead) %>% 
            CalculateMA(input$date)
    })
...
# DATA ANALYSIS -----------------------------------------------------------
    ...
    observe({
        fields <- if (is.null(input$regions)) {
            c("Admitted to Hospital" = "Hospitalised", 
...
# EPIDEMIOLOGICAL STUDY ---------------------------------------------------
    ...
    g1.data <- reactive({
        SelectCharacteristic(Data$covid.characteristics, "Sex") 
    })    
    
    output$g1 <- plotly::renderPlotly({
        PlotCharacteristic(g1.data())
    })
    
    output$g1.ts <- plotly::renderPlotly(
        PlotCharacteristicTimeSeries(g1.data())
    )
    
    g2.data <- reactive({
        SelectCharacteristic(Data$covid.characteristics, "HSE area") 
    })
    
    output$g2 <- plotly::renderPlotly({
        PlotCharacteristic(g2.data())
    })
    
    output$g2.ts <- plotly::renderPlotly(
        PlotCharacteristicTimeSeries(g2.data())
    )
    
    g3.data <- reactive({
        SelectCharacteristic(Data$covid.characteristics, "Transmission Type") 
    })
    
    output$g3 <- plotly::renderPlotly({
        PlotCharacteristic(g3.data())
    })
    
    output$g3.ts <- plotly::renderPlotly(
        PlotCharacteristicTimeSeries(g3.data())
    )
    
    g4.data <- reactive({
        SelectCharacteristic(Data$covid.characteristics, "Age Group",
                             input$g.field) 
    })
    
    output$g4 <- plotly::renderPlotly({
        PlotCharacteristic(g4.data())
    })
    
    output$g4.ts <- plotly::renderPlotly(
        PlotCharacteristicTimeSeries(g4.data())
    )
    
    g5.data <- reactive({
        SelectCharacteristic(Data$covid.characteristics, 
                             "Healthcare Workers Cases", input$g.workers) 
    })
    
    output$g5 <- plotly::renderPlotly({
        PlotCharacteristic(g5.data())
    })
    
    output$g5.ts <- plotly::renderPlotly(
        PlotCharacteristicTimeSeries(g5.data())
    )
})
